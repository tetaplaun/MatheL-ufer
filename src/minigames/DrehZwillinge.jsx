...
import { buildTwinPuzzle } from './buildPuzzle.js';
...
import styles from './DrehZwillinge.module.css';

const GAME_ID = 'drehzwillinge';
const ROUND_COUNT = 8;
const WRONG_MS = 420;

const now = () => (typeof performance !== 'undefined' ? performance.now() : Date.now());

export default function DrehZwillinge({ settings, onExit, onComplete }) {
  const reducedMotion = useReducedMotion();
  const scoreKey = useMemo(() => makeMiniGameScoreKey(GAME_ID, settings, { includeRoute: false }), [settings]);

  const [puzzle, setPuzzle] = useState(() => buildTwinPuzzle(settings));
  ...
  const nextPuzzle = useCallback(() => {
    setPuzzle(buildTwinPuzzle(settings));
  }, [settings]);
  ...
  const handleChoice = useCallback(
    (option) => {
      ...
      if (option.correct) {
        ...
        factorsRef.current.push(puzzle.a);
        if (puzzle.b !== puzzle.a) {
          factorsRef.current.push(puzzle.b);
        }
    ...
    [done, finish, nextPuzzle, puzzle.a, puzzle.b],
  );
  ...
  const status = (
    <div className={styles.status}>
      <span>
        Karte <strong>{Math.min(round, ROUND_COUNT)}</strong> / {ROUND_COUNT}
      </span>
      <span>
  ...
  return (
    <MiniGameShell title="Dreh-Zwillinge" icon="🔁" accent="#2f9b61" onExit={onExit} status={status}>
      {confettiId > 0 && <ConfettiBurst key={confettiId} pieces={confettiPieces} />}

      {done ? (
        <div className={styles.doneCard} role="status">
          <h3>Zwillinge gefunden!</h3>
          <p className={styles.doneStat}>
            {ROUND_COUNT} Karten in <strong>{result?.durationSeconds.toFixed(1)} s</strong>
          </p>
        ...
        <div className={styles.play}>
          <div className={styles.card} aria-live="polite">
            <span className={styles.cardLabel}>Dreh diese Aufgabe</span>
            <strong>
              {puzzle.a} × {puzzle.b}
            </strong>
          </div>

          <div className={styles.arrayProof} aria-hidden="true">
            <div className={styles.arrayBlock}>
              {Array.from({ length: Math.min(puzzle.a, 8) }, (_, rowIndex) => (
                <span key={rowIndex} style={{ '--dot-count': Math.min(puzzle.b, 8) }} />
              ))}
            </div>
            <span className={styles.turnArrow}>↻</span>
            <div className={`${styles.arrayBlock} ${styles.arrayBlockTurned}`}>
              {Array.from({ length: Math.min(puzzle.b, 8) }, (_, rowIndex) => (
                <span key={rowIndex} style={{ '--dot-count': Math.min(puzzle.a, 8) }} />
              ))}
            </div>
          </div>
          ...
          <div className={styles.options} data-count={settings.answerCount}>
            {puzzle.options.map((option) => (
              <button
                key={`${puzzle.a}-${puzzle.b}-${option.id}`}
                type="button"
          ...
          <div className={styles.microLayer}>
            {microBurst > 0 && <MicroReward key={microBurst} origin={{ x: 50, y: 58 }} star="★" />}
          </div>
        </div>
      )}
    </MiniGameShell>
